...
function getUserDevices(req, res) {
  ...
  dbapi.loadUserDevices(req.user.email)
    .then(function(cursor) {
      return Promise.promisify(cursor.toArray, {context: cursor})()
        .then(function(list) {
          var deviceList = []
...
function rebootUserDeviceBySerial(req, res) {
  var serial = req.swagger.params.serial.value
...
function getAccessTokens(req, res) {
  dbapi.loadAccessTokens(req.user.email).then(function(cursor) {
    Promise.promisify(cursor.toArray, {context: cursor})().then(function(tokens) {
      const tokenList = []
...
, rebootUserDeviceBySerial: rebootUserDeviceBySerial
